...
BASE = "https://api.github.com/repos/{}/contents/packages/player/rarity/{}".format(REPO, "{}")
PKG = "/code/ballsdex/packages/rarity"
...
def add_to_config():
    with open(CONFIG, "r") as f:
        config = f.read()
    if "ballsdex.packages.rarity" not in config:
...
def remove_from_config():
    with open(CONFIG, "r") as f:
        lines = f.readlines()
    lines = [l for l in lines if "ballsdex.packages.rarity" not in l]
    with open(CONFIG, "w") as f:
        f.writelines(lines)
...
def _remove_rarity_command(bot):
    """Remove the rarity command from the balls/objects group."""
    from ballsdex.settings import settings
    group_name = settings.players_group_cog_name
    group = bot.tree.get_command(group_name)
    if group and hasattr(group, "get_command"):
        if group.get_command("rarity"):
...
def build_confirm_embed() -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            "• Remove the rarity command from Discord\n"
...
class ConfirmDeleteView(View):
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: Button):
        ...
        DELETE_STEPS = [
            "Removing rarity command from Discord",
            "Unloading extension",
        ...
        try:
            _remove_rarity_command(self.parent.bot)
            await update(0)

            try:
                await self.parent.bot.unload_extension("ballsdex.packages.rarity")
            except Exception:
                pass
            await update(1)
...
class RarityInstallerView(View):
    ...
    async def install_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            await self.bot.load_extension("ballsdex.packages.rarity")
            await update(3)
    ...
    async def update_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            loaded = "ballsdex.packages.rarity" in self.bot.extensions
            if loaded:
                await self.bot.reload_extension("ballsdex.packages.rarity")
            else:
                await self.bot.load_extension("ballsdex.packages.rarity")
            await update(1)
