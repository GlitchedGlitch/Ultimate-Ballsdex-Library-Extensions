"""
ReSlasher Installer - BallsDex v3
"""
...
def build_main_embed(installed: bool, color: discord.Color) -> discord.Embed:
    ...
    e = discord.Embed(
        title="ReSlasher",
        description=(
            "Rename any slash command via the admin panel\n\n"
            "**Admin Panel**\n"
            "* Adds a **Command Names** page under Settings\n"
            "* Commands grouped alphabetically by parent group\n"
            "* Supports all commands including 3rd party packages\n\n"
            f"**Status:** {status}"
        ),
        color=color,
    )
...
def build_confirm_remove_embed() -> discord.Embed:
    e = discord.Embed(
        ...
        description=(
            ...
            "All saved command name overrides are preserved in the database "
            "and will be re-applied if you reinstall."
        ),
        color=discord.Color.orange(),
    )
    e.set_footer(text=FOOTER)
...
class InstallWarningView(View):
    def __init__(self, parent: "ReSlasherInstallerView"):
        ...
        try:
            ...
            await self.parent.message.edit(
                embed=build_result_embed(
                    ...
                    "Now rebuild and restart your bot:\n"
                    "```\ndocker compose build --no-cache\ndocker compose up -d\n```\n",
                    discord.Color.green(),
                ),
                view=None,
            ...
            await self.parent.message.edit(
                embed=build_result_embed(
                    "Permission Denied",
                    f"Could not write to `config/extra.toml` - still **read-only** (`{e.strerror}`).\n\n"
        ...
        try:
            ...
            await self.parent.message.edit(
                embed=build_result_embed(
                    ...
                    "```\ndocker compose build --no-cache\ndocker compose up -d\n```\n"
                    "Saved overrides are preserved in the database.",
                    discord.Color.red(),
                ),
                view=None,
            )
...
class ReSlasherInstallerView(View):
...
if _is_v2():
    await ctx.send(
        embed=discord.Embed(
            ...
            description="This installer is for **BallsDex v3** only.",
            color=discord.Color.red(),
        ).set_footer(text=FOOTER)
    )
else:
    installed = is_installed()
    view      = ReSlasherInstallerView(bot, ctx, installed)
