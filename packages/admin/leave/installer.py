...
BASE = "https://api.github.com/repos/GlitchedGlitch/Ultimate-Ballsdex-Library-Extensions/contents/packages/admin/leave/{}?ref=v2-main".format("{}")
...
def _remove_command(bot):
    ...
    if admin_cog and admin_cog.__cog_app_commands_group__:
        ...
        if group.get_command("leave_server"):
...
def build_main_embed(installed: bool, color: discord.Color) -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Commands**\n"
            "• `/admin leave_server` — make the bot leave a server\n\n"
            "**Parameters**\n"
            "• `server` — the ID of the server to leave\n\n"
...
class ConfirmDeleteView(View):
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            _remove_command(self.parent.bot)
            await update(0)
