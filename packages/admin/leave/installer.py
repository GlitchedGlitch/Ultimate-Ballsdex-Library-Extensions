"""
Leave Server Package Installer v3
"""
...
BRANCH      = "v3"
GIT_URL     = f"git+https://github.com/{REPO}.git@{BRANCH}#subdirectory=packages/admin/leave"
APP_PATH    = "leave"
...
def _remove_toml():
    ...
    with open(EXTRA_TOML) as f:
        contents = f.read()
    cleaned = re.sub(
        r"\n?# Leave Server Package\n\[\[ballsdex\.packages\]\][^\[]*path\s*=\s*\"leave\"[^\[]*",
...
def build_confirm_remove_embed() -> discord.Embed:
    e = discord.Embed(
        title="Remove Leave Server Package",
        description=(
            "⚠️ **Are you sure?**\n\n"
