...
up={};new=False;s=1.2;B="/code/admin_panel/media/"
...
def G(c, i):
    ...
    for k, pfx in (("wild_card", "spawn"), ("collection_card", "card")):
        p=getattr(c, k, None)
        if not p: continue
        ex=os.path.splitext(str(p))[1].lower()
        if ex not in E: ex=".png"
        o.append((str(p), f"{pfx}_{i}{ex}"))
    return o
...
balls=[b async for b in Ball.objects.all()]
for i, c in enumerate(balls, 1):
